...
function _zurb_foundation_render_link($link) {
  ...
  if (!empty($link['#href'])) {
    ...
    if (!empty($link['#below'])) {
      // Add repeated link under the dropdown for small-screen ** and medium-screen ** added by mi
      $small_link['#attributes']['class'][] = 'show-for-medium';
...
function zurb_foundation_preprocess_page(&$variables) {
  ...
  if ($variables['top_bar'] = theme_get_setting('zurb_foundation_top_bar_enable')) {
    ...
    if ($variables['top_bar'] == 2) {
      $top_bar_classes[] = 'show-for-medium'; //changed by mi small->medium
    }
  ...
  if ($variables['alt_header'] = ($variables['top_bar'] != 1)) {
    ...
    $variables['alt_header_classes'] = $variables['top_bar'] == 2 ? ' hide-for-medium' : ''; //changed by mi: small->medium
  }
